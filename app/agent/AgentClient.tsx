...
export default function AgentClient() {
  ...
  async function autoPublish() {
    ...
    const now = new Date();
const TODAY_DISPLAY = now.toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" });
const TODAY_ISO = now.toISOString().split("T")[0];
    ...
    for (let i = 0; i < 3; i++) {
      ...
      try {
        ...
        const res = await fetch("/api/agent", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              { role: "user", content: `Write a complete SEO-optimised article for bloxfruitsai.com about: ${topic}. Today's date is ${TODAY_DISPLAY}. Set the "date" field to exactly "${TODAY_DISPLAY}" — do not use any other date. Output ONLY raw JSON starting with { and ending with }.` },
            ],
          }),
        });
        ...
        post.date = TODAY_DISPLAY;
        post.dateISO = TODAY_ISO;
      ...
      <div style={{ display: "flex", height: "calc(100vh - 148px)" }}>
        ...
        <div style={{ width: 252, flexShrink: 0, borderRight: "1px solid rgba(0,245,255,0.08)", background: "rgba(6,15,30,0.5)", overflowY: "auto", padding: "1.1rem 0.85rem" }}>
          ...
          <div style={{ marginTop: "1rem", padding: "0.9rem", borderRadius: 9, border: "1px solid rgba(255,165,2,0.15)", background: "rgba(255,165,2,0.04)" }}>
            ...
            {[
              { label: "OPENAI_API_KEY set", done: true },
