...
import { FRUITS, formatValue, type FruitTier } from "../_data/fruits-data";
...
// ─── VALUE TIER TABLE DATA — computed live from FRUITS, never hardcoded ───────
const TIER_COLORS: Record<FruitTier, string> = {
  S: "#ff4757",
  A: "#ffd700",
  B: "#00f5ff",
  C: "#2ed573",
...
const tiers = (["S", "A", "B", "C"] as FruitTier[])
  .map((tier) => ({
    tier,
    color: TIER_COLORS[tier],
    fruits: FRUITS.filter((f) => f.tier === tier)
      .sort((a, b) => b.value - a.value)
      .slice(0, 3)
      .map((f) => ({
        name: f.name,
        bfv: formatValue(f.value),
        demand: f.demand,
        note: f.description,
      })),
  }))
  .filter((t) => t.fruits.length > 0);

const topByValue = [...FRUITS].sort((a, b) => b.value - a.value)[0];
const topByDemand = [...FRUITS].sort((a, b) => b.demand - a.demand)[0];
      ...
      <div style={{ background: "var(--bg-deep)", minHeight: "100vh", paddingTop: 90 }}>
        ...
        <section style={{ background: "var(--bg-card)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "40px 5%" }}>
          <div style={{ maxWidth: 820, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 20 }}>
            {[
              { label: "Fruits Tracked", value: "45+", sub: "all devil fruits" },
              { label: "Updated", value: "Daily", sub: "after every patch" },
              { label: "Top BFV Value", value: formatValue(topByValue.value), sub: `${topByValue.name} fruit` },
              { label: "Most Wanted", value: topByDemand.name, sub: `demand score: ${topByDemand.demand}/10` },
            ].map((s) => (
        ...
        <section style={{ maxWidth: 820, margin: "0 auto", padding: "60px 5%" }}>
          ...
          <p style={{ color: "var(--text-muted)", marginBottom: 32, fontSize: "0.95rem" }}>
            Top fruits by BFV trade value. For the full list of all 45 fruits see the{" "}
          ...
          <div style={{ textAlign: "center", marginTop: 16 }}>
            ...
            >
              View all 45 fruits on the full BFV value list →
            </Link>
          </div>
        </section>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            {[
              ...
              },
              {
                ...
                a: "Kitsune currently holds the highest BFV value, followed by Dragon, Leopard, and Venom. Exact numbers shift after every patch — check the live value list for current figures.",
              },
              {
